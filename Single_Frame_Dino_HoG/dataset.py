...
class Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        embedding = self.embedding[idx].to(self.device)
        label = torch.tensor(self.labels[idx]).to(self.device)

        return embedding, label

def process_CSV(model, method, csv_path, data_distribution):
    ...
    for index, row in df.iterrows():
        ...
        if len(action_frames) <= block_size:
            ...
            for i in range(0, len(action_frames), block_size):
                ...
                path = f'embeddings/{model}/{method}/{video_name}_{bird_id}_{start_frame}_{i}.pt'
            ...
            path = f'embeddings/{model}/{method}/{video_name}_{bird_id}_{start_frame}.pt'
            embedding = torch.load(path)
...
def get_dataloader(model, method, batch_size=32, shuffle=True, csv_path = '/dataset/annotations/crops.csv'):

    data_distribution = get_distribution()

    data = process_CSV(model, method, csv_path, data_distribution)
