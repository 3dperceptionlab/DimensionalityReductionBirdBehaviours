...
# MLP model to classify the embeddings, 400 input size, num classes by the constructor, parametrize with different dimensions, dropouts, num layers
class MLPModel(nn.Module):
    def __init__(self, input_size, num_classes, hidden_dim=512, dropout=0.2, num_layers=2):
        ...
        for _ in range(self.num_layers - 1):
            self.fc.add_module('fc', nn.Sequential(
                nn.Linear(self.hidden_dim, self.hidden_dim),
                nn.ReLU(),
                nn.Dropout(self.dropout)
            ))

        self.fc.add_module('output', nn.Linear(self.hidden_dim, self.num_classes))
